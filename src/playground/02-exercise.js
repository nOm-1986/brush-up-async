/*
En este desafío tienes la función delay la cual se espera que un tiempo específico retorne 
un mensaje
...
const delay = (time, message) => {
    return new Promise((resolve, reject) => {
        if(time > 0) {
            ...
            reject('Error, function without delay');
        }
    });
}

delay(0, "Hello after 2s").then((result) => {
    console.log(result);
}).catch((err) => {
    console.log(err);
});